...
test_skeleton.py: 包能 import + 9 个核心模块在场 (含 handoff) + CLI 入口可加载 + __version__ 校对。
...
test_handoff.py: write_drift_handoff / write_stale_handoff / write_quality_handoff 行为：文件输出含 disclaimer + task header + 每个 group/skill 一节；空集 defensive；diff > 50 行触发 structural divergence 标记。
legacy/: V0.1 旧测试，pytest norecursedirs 跳过。

法则: 夹具真实·断言行为·禁止触碰用户 skills·monkeypatch BACKUP_ROOT / HANDOFF_ROOT 隔离
